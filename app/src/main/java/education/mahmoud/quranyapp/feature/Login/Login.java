    ...
    public void onViewClicked() {
        ...
        if (checkInput(email)) {
            if (checkInput(password)) {
                StartOperationState();
                RxFirebaseAuth.signInWithEmailAndPassword(repository.getAuth(), email, password)
                        .subscribe(logged -> {
                            FinishOperationState();
                            try {
                                Log.d(TAG, "onBtnResgisterClicked:  logged " + logged.getUser().getDisplayName());
                                repository.setUserName(logged.getUser().getDisplayName());
                                repository.setUserUUID(logged.getUser().getUid());

                                showMessage(getString(R.string.logged_success));
                                Intent openAcivity = new Intent(Login.this, Home.class);
